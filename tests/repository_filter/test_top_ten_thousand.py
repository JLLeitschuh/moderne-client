...
from moderne_client.repository_filter.top_ten_thousand import TopTenThousandProjects
...
def test_load_top_ten_thousand():
    ...
    filter_details = top_ten_thousand.should_filter_repository(
        Repository(**{'origin': 'github.com', 'path': 'JLLeitschuh/moderne-client', 'branch': 'main'})
    )
    assert len(filter_details) == 0
    filter_details = top_ten_thousand.should_filter_repository(
